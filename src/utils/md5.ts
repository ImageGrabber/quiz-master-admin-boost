/**
 * Simple MD5 implementation for CPX Research hash generation.
 * This is a standard implementation found in various open-source libraries.
 */
export function md5(string: string) {
  ...
  function md5_Convert(s: string) {
    let words = [], i;
    for (i = 0; i < 64; i += 4) {
      words[i >> 2] = s.charCodeAt(i) + (s.charCodeAt(i + 1) << 8) + (s.charCodeAt(i + 2) << 16) + (s.charCodeAt(i + 3) << 24);
    }
    return words;
  }
