...
const nextQuizzes = [
  ...
  { title: "Zephaniah Quiz", description: "Go back to the day of the Lord.", link: "/bible-questions-and-answers-hub/zephaniah" },
  { title: "Habakkuk Quiz", description: "Continue to faith in troubled times.", link: "/bible-questions-and-answers-hub/habakkuk" }
];

const haggaiQuestions = [
  { chapter: 1, question: "What did Haggai urge the people to do?", options: ["Rebuild the temple", "Flee the land", "Fight their enemies", "Rest"], answer: 0 },
  { chapter: 1, question: "Why was there a drought?", options: ["Because of the people's neglect", "Because of their enemies", "Because of the weather", "Because of the king"], answer: 0 },
  { chapter: 2, question: "Who was the governor of Judah?", options: ["Zerubbabel", "Nehemiah", "Ezra", "Joshua"], answer: 0 },
  { chapter: 2, question: "Who was the high priest?", options: ["Joshua", "Zerubbabel", "Ezra", "Nehemiah"], answer: 0 },
  { chapter: 2, question: "What did God promise to do with the temple?", options: ["Fill it with glory", "Destroy it", "Ignore it", "Curse it"], answer: 0 },
  { chapter: 2, question: "What did God promise to do for Zerubbabel?", options: ["Make him like a signet ring", "Make him king", "Make him priest", "Make him prophet"], answer: 0 }
];

export default function HaggaiQuiz() {
  const [selected, setSelected] = useState<(number | null)[]>(Array(haggaiQuestions.length).fill(null));
  const [showAnswer, setShowAnswer] = useState<boolean[]>(Array(haggaiQuestions.length).fill(false));
  ...
  const correctCount = selected.filter((sel, i) => sel !== null && sel === haggaiQuestions[i].answer).length;
  const wrongCount = selected.filter((sel, i) => sel !== null && sel !== haggaiQuestions[i].answer).length;
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row gap-8">
        <div className="flex-1 min-w-0">
          ...
          <Card className="mb-8 shadow-lg border-0">
            <CardHeader>
              <CardTitle>Haggai Quiz</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-8">
                {haggaiQuestions.map((q, qIdx) => (
