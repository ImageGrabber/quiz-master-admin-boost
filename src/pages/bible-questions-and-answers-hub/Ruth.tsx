...
const nextQuizzes = [
  ...
  { title: "Judges Quiz", description: "Go back to the time of the judges.", link: "/bible-questions-and-answers-hub/judges" },
  { title: "Joshua Quiz", description: "Review the conquest of Canaan.", link: "/bible-questions-and-answers-hub/joshua" }
];

const ruthQuestions = [
  { chapter: 1, question: "Where did Naomi and her family go during the famine?", options: ["Egypt", "Moab", "Canaan", "Philistia"], answer: 1 },
  { chapter: 1, question: "Who were Naomi's two daughters-in-law?", options: ["Ruth and Orpah", "Rachel and Leah", "Sarah and Hagar", "Miriam and Deborah"], answer: 0 },
  { chapter: 1, question: "What did Naomi tell her daughters-in-law to do?", options: ["Stay with her", "Return to their mothers", "Go to Bethlehem", "Find new husbands"], answer: 1 },
  { chapter: 1, question: "Who stayed with Naomi?", options: ["Orpah", "Ruth", "Both", "Neither"], answer: 1 },
  { chapter: 2, question: "Who was Boaz?", options: ["A relative of Naomi", "A stranger", "A priest", "A king"], answer: 0 },
  { chapter: 2, question: "What was Ruth doing when Boaz first saw her?", options: ["Praying", "Gleaning in the field", "Cooking", "Resting"], answer: 1 },
  { chapter: 2, question: "What did Boaz tell his workers about Ruth?", options: ["Ignore her", "Help her", "Drive her away", "Marry her"], answer: 1 },
  { chapter: 3, question: "What did Naomi tell Ruth to do at the threshing floor?", options: ["Work there", "Sleep there", "Hide there", "Dance there"], answer: 1 },
  { chapter: 3, question: "What did Ruth uncover at Boaz's feet?", options: ["His head", "His hands", "His feet", "His face"], answer: 2 },
  { chapter: 4, question: "Who was the closer relative to Naomi?", options: ["Boaz", "The unnamed relative", "Elimelech", "Mahlon"], answer: 1 },
  { chapter: 4, question: "What did the closer relative give up to Boaz?", options: ["His land", "His right to redeem", "His wife", "His money"], answer: 1 },
  { chapter: 4, question: "Who was Ruth and Boaz's son?", options: ["Obed", "Jesse", "David", "Solomon"], answer: 0 }
];

export default function RuthQuiz() {
  const [selected, setSelected] = useState<(number | null)[]>(Array(ruthQuestions.length).fill(null));
  const [showAnswer, setShowAnswer] = useState<boolean[]>(Array(ruthQuestions.length).fill(false));
  ...
  const correctCount = selected.filter((sel, i) => sel !== null && sel === ruthQuestions[i].answer).length;
  const wrongCount = selected.filter((sel, i) => sel !== null && sel !== ruthQuestions[i].answer).length;
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <Card className="mb-8 shadow-lg border-0">
            <CardHeader>
              <CardTitle>Ruth Quiz</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-8">
                {ruthQuestions.map((q, qIdx) => (
