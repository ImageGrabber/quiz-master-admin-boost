...
const nextQuizzes = [
  ...
  { title: "Nahum Quiz", description: "Go back to the prophecy against Nineveh.", link: "/bible-questions-and-answers-hub/nahum" },
  { title: "Micah Quiz", description: "Continue to justice and mercy.", link: "/bible-questions-and-answers-hub/micah" }
];

const habakkukQuestions = [
  { chapter: 1, question: "What did Habakkuk complain to God about?", options: ["Violence and injustice", "Poverty", "Idolatry", "Famine"], answer: 0 },
  { chapter: 1, question: "Who did God say He would raise up?", options: ["Babylonians", "Assyrians", "Egyptians", "Philistines"], answer: 0 },
  { chapter: 2, question: "What does God say the righteous will live by?", options: ["Works", "Faith", "Sacrifice", "Law"], answer: 1 },
  { chapter: 2, question: "What does Habakkuk say about idols?", options: ["They are powerful", "They are worthless", "They are helpful", "They are beautiful"], answer: 1 },
  { chapter: 3, question: "What does Habakkuk do in his prayer?", options: ["Complains", "Praises God", "Fights", "Flees"], answer: 1 },
  { chapter: 3, question: "How does Habakkuk end his book?", options: ["With despair", "With hope and faith", "With anger", "With confusion"], answer: 1 }
];

export default function HabakkukQuiz() {
  const [selected, setSelected] = useState<(number | null)[]>(Array(habakkukQuestions.length).fill(null));
  const [showAnswer, setShowAnswer] = useState<boolean[]>(Array(habakkukQuestions.length).fill(false));
  ...
  const correctCount = selected.filter((sel, i) => sel !== null && sel === habakkukQuestions[i].answer).length;
  const wrongCount = selected.filter((sel, i) => sel !== null && sel !== habakkukQuestions[i].answer).length;
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row gap-8">
        <div className="flex-1 min-w-0">
          ...
          <Card className="mb-8 shadow-lg border-0">
            <CardHeader>
              <CardTitle>Habakkuk Quiz</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-8">
                {habakkukQuestions.map((q, qIdx) => (
