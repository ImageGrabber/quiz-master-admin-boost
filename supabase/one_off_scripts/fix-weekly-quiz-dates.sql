-- Fix Weekly Quiz Dates
-- This script updates the existing weekly quiz (ID 11) to have the current week's dates
-- OR creates a new quiz for the current week if needed
...
BEGIN
  ...
  -- If no quiz exists for current week, update the existing quiz (ID 11) or create new one
  IF quiz_id IS NULL THEN
    -- Try to update quiz ID 11 if it exists
    ...
    IF quiz_id IS NULL THEN
      -- Quiz ID 11 doesn't exist, create a new one
      ...
      END IF;
    ELSE
      RAISE NOTICE 'Updated quiz ID 11 with current week dates: % to %', current_week_start, current_week_end;
    END IF;
  ELSE
    RAISE NOTICE 'Quiz already exists for current week with ID: %', quiz_id;
  END IF;
END $$;
